...
from cotton_factor.ingest.czce_settlement_param import (
    SOURCE_NAME,
    ingest_czce_settlement_param,
)
...
@pytest.mark.parametrize(
    ...
    [
        ("czce_settlement_param_sample.csv", "text/csv"),
        ("czce_settlement_param_sample.html", "text/html"),
    ],
)
def test_czce_settlement_param_ingests_fixture_as_raw_snapshot(
    tmp_path: Path,
    fixture_name: str,
    content_type: str,
) -> None:
    ...
    assert snapshot.parser_version == "czce_settlement_param.raw.v1"
    ...
    assert (
        snapshot.metadata["settlement_param_roles"]
...
def test_czce_settlement_param_requires_fixture_when_network_disabled(tmp_path: Path) -> None:
